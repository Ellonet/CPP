...
void testVector()
{
	...
	buffer << t;
	check(buffer, "0 0 0", "Construct a new _vector with default constructor");

	buffer << a;
	check(buffer, "3 2 5", "Construct a new _vector with 3 given doubles");

	buffer << c;
	check(buffer, "5 1 4", "Construct a new _vector with array of 3 doubles");

	Vector3D r(c);
	buffer << r;
	check(buffer, "5 1 4", "Construct a new _vector with copy constructor");
	...
	std::cin.rdbuf(orig);
	buffer << f;
	check(buffer, "5 4 3", "Test '>>' operator. Read \"5 4 3\" from stdin into _vector");
