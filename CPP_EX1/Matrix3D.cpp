...
Matrix3D::Matrix3D(double (&arr)[3][3]) {
    Vector3D
    v1(arr[0][0], arr[0][1], arr[0][2]);
    Vector3D
    v2(arr[1][0], arr[1][1], arr[1][2]);
    Vector3D
    v3(arr[2][0], arr[2][1], arr[2][2]);
    _init(v1, v2, v3);
}
 ...
 */
    Vector3D
    Matrix3D::operator*(Vector3D & vector)
    {
        Vector3D
        res;
 ...
 */
    Vector3D & Matrix3D::operator[](int
    index)
    {
        if (index == 0) {
            return this->v1;
